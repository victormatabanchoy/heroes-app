...
export const fetchHeroesFromApi = async (buscar) => {
  try {
...
//filtro
    const matchedHeroes = apiHeroes.filter(
      (apiHero) =>
        apiHero.name.toLowerCase().includes(searchTerm) ||
        (apiHero.biography.fullName &&
          apiHero.biography.fullName.toLowerCase().includes(searchTerm))
    );
